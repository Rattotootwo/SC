...
本文档面向本仓库 `tracking` 示例，目标是让你快速完成：
- 环境安装与资源准备
- 各模式运行
- `core` 数据接口模板接入与导出
- 代码结构与每个代码文件职责理解
...
- `setup.py`
  - Python 包构建入口。
  - 使用 `setuptools.find_packages()` 自动收集 `tracking` 与 `core` 包。
...
- `tracking/configs/soccer.py`
  - 定义足球场几何配置：尺寸、关键点顶点、连线、标注标签和颜色。

- `tracking/annotators/soccer.py`
  - 负责在“俯视球场图”上绘制场地、点、轨迹。
  - 主要用于 `RADAR` 模式可视化。

- `tracking/common/view.py`
  - 透视变换工具 `ViewTransformer`。
  - 支持点坐标变换与整图变换（单应矩阵）。

- `tracking/common/ball.py`
  - `BallTracker`：基于历史中心点做简单球目标稳定选择。
  - `BallAnnotator`：在视频帧上绘制球轨迹样式。

- `tracking/common/team.py`
  - `TeamClassifier`：SigLIP 特征提取 + UMAP 降维 + KMeans 聚类做球队分类。
  - `create_batches`：批处理工具函数。

- `tracking/__init__.py`、`tracking/common/__init__.py`、`tracking/configs/__init__.py`、`tracking/annotators/__init__.py`
  - 包初始化文件（当前无复杂逻辑）。
...
- `tracking/setup.sh`
  - 下载模型权重与示例视频到 `tracking/data/`。
  - 依赖 `gdown`。

- `tracking/requirements.txt`
  - 示例额外依赖：`ultralytics`、`gdown`。

- `tracking/notebooks/*.ipynb`
...
说明：
- `pip install -e .` 会安装当前仓库源码（包含 `tracking` 和 `core`）。
- `./setup.sh` 会拉取模型与视频到 `tracking/data/`。
...
- 本仓库 `tracking` 代码遵循 MIT（见根目录 `LICENSE`）。
